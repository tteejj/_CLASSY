...
function Start-PMCTerminal {
    ...
    Invoke-WithErrorHandling -ScriptBlock {
        ...
        # AI: Check if navigation service has route validation
        if ($services.Navigation -and (Get-Member -InputObject $services.Navigation -Name "IsValidRoute" -ErrorAction SilentlyContinue)) {
            ...
            # Fallback navigation
            Write-Log -Level Info -Message "Using fallback navigation to initial screen"
            if ($services.Navigation -and (Get-Member -InputObject $services.Navigation -Name "PushScreen" -ErrorAction SilentlyContinue)) {
